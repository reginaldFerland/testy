...
Prepared outputs are cached between runs within the current editor session. Testy validates the complete build output, preparation tools, and configuration before reusing instrumentation, and runs fresh discovery on every run. The cache retains up to 16 preparations or 512 MiB, evicting the least recently used idle entries; active runs may temporarily exceed those limits. Shutdown drains active work and removes retained preparations. Outputs left by a terminated host are reclaimed on the next startup. Saved coverage still supplies history while startup establishes a fresh baseline.
