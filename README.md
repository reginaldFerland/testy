...
Prepared outputs are cached between runs and can survive a clean editor shutdown. Testy validates the complete build output, preparation tools, environment, and configuration before reusing instrumentation, and runs fresh discovery on every run. Shutdown drains active work, deletes working copies and reports, and retains only pristine templates. Another editor instance can claim those templates exclusively; live instances never share mutable outputs or collector sessions. Active caches and the combined retained templates each have limits of 16 preparations or 512 MiB, with active work temporarily exempt. Least recently used idle entries are evicted, and outputs from crashed active owners are discarded. Startup still performs build checks and establishes a fresh baseline; saved coverage supplies history while that runs. Custom collectors retain their complete environment, so editor environment changes can invalidate their preparations.

Project evaluations can also be reused within the editor session after validating the files, existence checks, and directory searches MSBuild used. Changes to imports, SDKs, file inventories, configuration, or evaluation tools require fresh evaluation. Unsupported custom evaluation logic is evaluated normally. Builds continue to use MSBuild's incremental checks.
