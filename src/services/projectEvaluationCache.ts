...
interface Entry { readonly graph: readonly Project[]; readonly inputs: readonly EvaluationInputs[]; readonly context: string; readonly stamp: string; }
...
/** Session-only graph reuse: every hit validates evaluated files and actual
 * MSBuild filesystem queries, including absent imports and glob inventories. */
export class ProjectEvaluationCache {
    private readonly entries = new Map<string, Entry>();
    retain(files: readonly string[]): void {
        const live = new Set(files);
        for (const file of this.entries.keys()) {if (!live.has(file)) {this.entries.delete(file);}}
    }
    async get(files: readonly string[], context: string, signal?: AbortSignal): Promise<ReadonlyMap<string, readonly Project[]>> {
        const validation = new Validation(signal), result = new Map<string, readonly Project[]>();
        await mapConcurrent(files, 4, signal, async file => {
            const entry = this.entries.get(file);
            if (!entry || entry.context !== context) {return;}
            try {if (await validation.stamp(entry.graph, entry.inputs, signal) === entry.stamp) {result.set(file, entry.graph);}}
    ...
    async put(graphs: ReadonlyMap<string, readonly Project[]>, context: string, signal?: AbortSignal, dotnetRoot?: string): Promise<void> {
        const validation = new Validation(signal);
        await mapConcurrent([...graphs], 4, signal, async ([file, graph]) => {
            ...
            try {
                ...
                if (stamp) {this.entries.set(file, { graph, inputs, context, stamp });}
